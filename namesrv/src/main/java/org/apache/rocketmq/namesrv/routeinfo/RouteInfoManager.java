...
public class RouteInfoManager {
    ...
    */
    ...
    public RegisterBrokerResult registerBroker(
        ...
        try {
            try {
                ...
                // 判断Broker所属集群是否存在，如果不存在，则创建集群，然后将broker名加入集群Broker集合
                if (null == brokerNames) {
                ...
                // 第二步：维护BrokerData信息，首先从brokerAddrTable中根据broker名尝试获取Broker信息，
                // 如果不存在，则新建BrokerData并放入brokerAddrTable，registerFirst设置为true；如果存在，直接替换原先的Broker信息，registerFirst设置为false，表示非第一次注册
                ...
                // 第三步：如果Broker为主节点，并且Broker的topic配置信息发生变化或者是初次注册，则需要创建或更新topic路由元数据，并填充topicQueueTable
                // 其实就是为默认主题自动注册路由信息，其中包含MixAll.DEFAULT_TOPIC的路由信息。当消息生产者发送主题时，如果该主题未创建，并且BrokerConfig的autoCreateTopicEnable为true，则返回MixAll.DEFAULT_TOPIC的路由信息
                ...
                // 第四步：更新BrokerLiveInfo，存储状态正常的Broker信息表，BrokeLiveInfo是执行路由删除操作的重要依据
    ...
    /***
     *
     * 创建或更新topic路由元数据  根据topicConfig创建QueueData数据结构，然后更新topicQueueTable
     * @param brokerName
     * @param topicConfig
     */
    ...
    /***
     * 扫描非存活的broker 每10s执行一次
     * 遍历brokerLiveInfo路由表（HashMap），检测BrokerLiveInfo的LastUpdateTimestamp上次收到心跳包的时间，如果超过120s，则认为该Broker已不可用，然后将它移除并关闭连接，最后删除与该Broker相关的路由信息
     */
    ...
    public void onChannelDestroy(String remoteAddr, Channel channel) {
        ...
        // brokerLiveTable 获取 brokerAddrFound
        if (channel != null) {
            try {
                try {
                    //  读锁
        ...
        if (brokerAddrFound != null && brokerAddrFound.length() > 0) {
            ...
            try {
                try {
                    // 第一步：申请写锁。根据brokerAddress从brokerLiveTable、filterServerTable中移除Broker相关的信息
                    this.lock.writeLock().lockInterruptibly();
                    this.brokerLiveTable.remove(brokerAddrFound);
                    this.filterServerTable.remove(brokerAddrFound);
                    //  第二步：维护brokerAddrTable。遍历HashMap<String/* brokerName */, BrokerData> brokerAddrTable，
                    ...
                    while (itBrokerAddrTable.hasNext() && (null == brokerNameFound)) {
                        ...
                        if (brokerData.getBrokerAddrs().isEmpty()) {
                            removeBrokerName = true;
                            // 在brokerAddrTable中移除该brokerName对应的条目
                    ...
                    // 第三步：根据BrokerName，从clusterAddrTable中找到Broker并将其从集群中移除。如果移除后，集群中不包含任何Broker，则将该集群从clusterAddrTable中移除
                    if (brokerNameFound != null && removeBrokerName) {
                        ...
                        while (it.hasNext()) {
                            ...
                            // 根据BrokerName，从clusterAddrTable中找到Broker并将其从集群中移除
                            boolean removed = brokerNames.remove(brokerNameFound);
                            if (removed) {
                                log.info("remove brokerName[{}], clusterName[{}] from clusterAddrTable, because channel destroyed",
                                    brokerNameFound, clusterName);
                                // 如果移除后，集群中不包含任何Broker
                                if (brokerNames.isEmpty()) {
                                    log.info("remove the clusterName[{}] from clusterAddrTable, because channel destroyed and no broker in this cluster",
                                        clusterName);
                                    // 将该集群从clusterAddrTable中移除
                                    it.remove();
                                }
                    ...
                    // 第四步：根据BrokerName，遍历所有主题的队列，如果队列中包含当前Broker的队列，则移除，如果topic只包含待移除Broker的队列，从路由表中删除该topic
                    if (removeBrokerName) {
